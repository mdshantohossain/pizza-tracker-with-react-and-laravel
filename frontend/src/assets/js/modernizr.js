 ...
 (function ($, window, document, undefined) {
    ...
    GlassCase.prototype = {
        init: function (options) {
            ...
            var ctntDisplayArea = "<div class='gc-display-area'>" +
                                        ...
                                        "<div class='gc-display-container'>" +
                                            ...
                                            "<images class='gc-display-display' alt=' ' />" +
                                        "</div>" +
                                     "</div>";
            var ctntZoomArea = "<div class='gc-zoom'>" +
                                        "<div class='gc-zoom-container'><images alt=' ' /></div>" +
                                     "</div>";
                                    ...
                                    "<div class='gc-overlay-gcontainer'>" +
                                        "<div class='gc-overlay-container'>" +
                                            "<div class='gc-overlay-container-display'>" +
                                                "<images class='gc-overlay-display' alt=' ' />" +
                                            "</div>" +
                                        "</div>" +
                                    "</div>" +
                                 "</div>";
            ...
            gcBase.gcZoomDisplay = gcBase.gcZoomContainer.find('images');
            ...
            gcBase.gcThumbsLi.each(function (index) {
                var iSrc = $.trim($(this).find('images').attr('src'));
                var els = gcBase.gcThumbsLi.find('images[src="' + iSrc + '"]');

                while (els.length > 1) {
                    els.last().parent().remove();
                    gcBase.gcThumbsLi = gcBase.gcThumbsUl.find('li');
                    els = gcBase.gcThumbsLi.find('images[src="' + iSrc + '"]');
                }
            });
            gcBase.gcThumbsLi = gcBase.gcThumbsUl.find('li');
            gcBase.gcThumbsImg = gcBase.gcThumbsLi.find('images');
            ...
            var altTxt = gcBase.gcThumbsLi.eq(gcBase.current).find('images').attr('alt');
				    ...
				    gcBase.gcThumbsImg.each(function (index) {
                        ...
                        .on('load', function () {
                            var lWidth = this.width,
                                ...
                                index = gcBase.gcThumbsLi.find("images[src*='" + $(this).attr('src') + "']").parents('li').index();
                            gcBase.gcImageData[index] = lGCImageData;
                            gcBase.processThumbImage(index);
                            if (++countLoadedImages === countTotalImages) { dfd.resolve(); }
                        })
                        .on('error', function () {
                            var index = gcBase.gcThumbsLi.find("images[src*='" + $(this).attr('src') + "']").parents('li').index(),
        ...
        update: function () {
            ...
            //2.
            altTxt = gcBase.gcThumbsLi.eq(gcBase.current).find('images').attr('alt');
            if (altTxt === undefined) altTxt = 'image';

            gcBase.gcDisplayDisplay.attr('src', gcBase.gcThumbsLi.eq(gcBase.current).find('images').attr('src'))
                                   .attr('alt', altTxt);
